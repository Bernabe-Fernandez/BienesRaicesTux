...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["option"] == '1') {
    ...
    if (empty($errores)) {
        ...
        if ($resultado) {
            ...
            echo "<script language=\"javascript\">window.location.href=\"index.php?resultado=2\";</script>";
        }
    }
    ...
    var_dump($query);
        ...
        if ($resultado) {
            //REDICCIONAR AL USUARIO
            echo "<script language=\"javascript\">window.location.href=\"index.php?resultado=1\";</script>";
        }
    }
}
